...
client_id = f'publishmqtt-{random.randint(0, 1000)}'
...
def subscribe(client: mqtt):
    def on_message(client, userdata, msg):
        ...
        if msg.topic == TOPIC_TERIMA_PESAN:
            status_video = msg.payload.decode()
            print("Status Video On Message: ", status_video)
        
    client.subscribe(TOPIC_TERIMA_PESAN)
...
global status_video
status_video = "video on"  # Nilai awal variabel status_video
...
def send_telegram_photo_from_frame(frame, token, chat_id, caption):
    ...
    global num_seconds
    global status_video
    print(f"Status Video: {status_video}")
    
    if num_person >= 1 and status_video == "video on":
        num_seconds = 100
